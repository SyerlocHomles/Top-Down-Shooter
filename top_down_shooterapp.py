...
st.set_page_config(page_title="Island.io: Kaboom!", layout="centered")
...
if not st.session_state.char:
    st.info("Pilih karakter untuk memulai pertempuran!")
    st.stop()

c = st.session_state.char
st.success(f"Bertarung sebagai: **{c['name']}**")

# Logika Game (HTML & JS)
# Simbol {{ }} digunakan agar Python f-string tidak error
game_html = f"""
...
<script>
    ...
    let sc=0, li={c['hp']}, go=false, ks={{}}, buls=[], enms=[], wls=[], itms=[], pX=[], boss=null, sk=0;
    ...
    function update() {{
        ...
        if(enms.length < 5) {{
            ...
            if(!checkCol(rx,ry,15,wls) && Math.sqrt((rx-ply.x)**2+(ry-ply.y)**2)>150){{
                let r=Math.random(), t=r<0.5?{{c:'#e74c3c',s:20,sp:1.6,h:5,sc:5}}:(r<0.8?{{c:'#2ecc71',s:14,sp:2.0,h:3,sc:10}}:{{c:'#9b59b6',s:35,sp:0.8,h:15,sc:20}});
        ...
        buls.forEach((b,i)=>{{
            b.x+=b.vx; b.y+=b.vy;
            if(checkCol(b.x,b.y,4,wls) || b.x<0 || b.x>600 || b.y<0 || b.y>400) {{ buls.splice(i,1); return; }}
            enms.forEach((e,ei)=>{{
                if(b.x>e.x && b.x<e.x+e.s && b.y>e.y && b.y<e.y+e.s){{
                    e.h-=5; buls.splice(i,1);
                    spawnExplosion(b.x, b.y, "#ffa500", 6);
                    if(e.h<=0){{ 
                        sc+=e.sc; 
                        spawnExplosion(e.x+e.s/2, e.y+e.s/2, e.c, 20); 
                        enms.splice(ei,1); sk=8; 
                        if(sc%100===0 && !boss) {{ boss={{x:300,y:50,s:60,h:400,mH:400,sp:0.8,fT:0}}; initMap(); }}
                    }}
                }}
            }});
        }});
        ...
        enms.forEach(e=>{{
            ...
            if(ply.inv<=0 && Math.sqrt((e.x+e.s/2-ply.x)**2+(e.y+e.s/2-ply.y)**2)<(e.s/2+ply.s)){{ 
                li--; ply.inv=60; sk=20; 
                spawnExplosion(ply.x, ply.y, "#ff0000", 25); 
                if(li<=0) go=true; 
            }}
        }});
        ...
        stB.innerHTML = "Skor: " + sc + " | Nyawa: " + hText;
    }}
    ...
    function draw() {{
        ...
        pX.forEach(p=>{{ ctx.fillStyle=p.col; ctx.globalAlpha=p.life/35; ctx.fillRect(p.x,p.y,p.s,p.s); }});
        ctx.globalAlpha=1;
    ...
    initMap(); 
    setInterval(()=>{{ if(itms.length<2) itms.push({{x:Math.random()*540+30, y:Math.random()*340+30, t:Math.random()<0.5?'speed':'triple'}}); }}, 6000); 
    draw();
</script>
"""
