...
<script>
    ...
    function fixS(e){{
        let t=0; while(col(e.x,e.y,e.s,wls) && t<50){{ e.x+=(Math.random()-0.5)*20; e.y+=(Math.random()-0.5)*20; t++; }}
    }}
    ...
    function spB(){{ boss={{x:300,y:50,s:60,h:350,mH:350,sp:0.8,fT:0,sT:0,sh:false}}; shufMap(); }}

    function init(){{ shufMap(); for(let i=0; i<5; i++) spE(); setInterval(() => {{ if(itms.length<2) itms.push({{x:Math.random()*540+30, y:Math.random()*340+30, t:Math.random()<0.5?'speed':'triple'}}); }}, 7000); }}
    ...
    function upd(){{
        ...
        itms.forEach((it,i)=>{{ if(Math.sqrt((it.x-ply.x)**2+(it.y-ply.y)**2) < 25){{ ply.pw=it.t; ply.pT=500; itms.splice(i,1); }} }});
